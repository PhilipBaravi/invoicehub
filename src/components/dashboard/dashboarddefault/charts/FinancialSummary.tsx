  ...
  const fetchFinancialSummary = async () => {
    try {
      const response = await fetch(
        "https://api.invoicehub.space/api/v1/dashboard/financialSummary",
        {
          headers: {
      ...
      if (result.success && result.data) {
        ...
        throw new Error(
          result.message || "Failed to fetch financial summary data"
        );
      }
    } catch (error) {
      console.error(
        "An error occurred while fetching financial summary data:",
        error
      );
      setError(
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        ...
        <CardDescription>
          Summary of costs, profits/losses, and sales
        </CardDescription>
      </CardHeader>
      <CardContent>
        {loading ? (
