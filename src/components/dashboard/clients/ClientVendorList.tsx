...
export default function ClientVendorList() {
  ...
  useEffect(() => {
    const fetchClientVendors = async () => {
      try {
        const response = await fetch('https://invoicehub-lb-1106916193.us-east-1.elb.amazonaws.com/api/v1/clientVendor/list', {
          headers: {
  ...
  const deleteClientVendor = async (id: number) => {
    try {
      const response = await fetch(`https://invoicehub-lb-1106916193.us-east-1.elb.amazonaws.com/api/v1/clientVendor/delete/${id}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${keycloak.token}`,
        },
