...
export default function ClientVendorList() {
  ...
  const [filteredClientVendors, setFilteredClientVendors] = useState<
    ClientVendor[]
  >([]);
  ...
  const [filterCategory, setFilterCategory] =
    useState<keyof ClientVendor>("name");
  ...
  useEffect(() => {
    const filtered = clientVendors.filter((clientVendor) => {
      const value =
        clientVendor[filterCategory]?.toString().toLowerCase() ?? "";
  ...
  const deleteClientVendor = async (id: number) => {
    try {
      ...
      if (!response.ok) {
        ...
        if (
          response.status === 409 &&
          data.message === "Client has invoice(s) and cannot be deleted."
        ) {
          toast({
  ...
  const handleSelectAll = () => {
    if (selectedClientVendors.length === filteredClientVendors.length) {
      ...
      setSelectedClientVendors(
        filteredClientVendors.map((cv) => cv.id.toString())
      );
    }
  };
