...
export default function ClientVendorList() {
  ...
  useEffect(() => {
    const fetchClientVendors = async () => {
      try {
        const response = await fetch('https://3.120.116.20:9090/api/v1/clientVendor/list', {
          headers: {
  ...
  const deleteClientVendor = async (id: number) => {
    try {
      const response = await fetch(`https://3.120.116.20:9090/api/v1/clientVendor/delete/${id}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${keycloak.token}`,
        },
