...
export default function EmployeeList() {
  ...
  useEffect(() => {
    const fetchEmployees = async () => {
      try {
        const response = await fetch('https://invoicehub-lb-1106916193.us-east-1.elb.amazonaws.com/api/v1/user/list', {
          headers: {
  ...
  const deleteEmployee = async (id: string) => {
    try {
      const response = await fetch(`https://invoicehub-lb-1106916193.us-east-1.elb.amazonaws.com/api/v1/user/delete/${id}`, {
        method: 'DELETE',
        headers: {
          Authorization: `Bearer ${keycloak.token}`,
        },
