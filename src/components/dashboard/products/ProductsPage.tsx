...
const ProductsPage: FC = () => {
  ...
  const fetchProducts = async () => {
    try {
      const response = await fetch("https://3.120.116.20:9090/api/v1/product/list", {
        method: "GET",
        headers: {
  ...
  const handleAddProduct = async () => {
    ...
    try {
      const response = await fetch("https://3.120.116.20:9090/api/v1/product/create", {
        method: "POST",
        headers: {
  ...
  const handleEditProduct = async () => {
    if (editingProduct && editingProduct.id) {
      try {
        const response = await fetch(`https://3.120.116.20:9090/api/v1/product/update/${editingProduct.id}`, {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${keycloak.token}`,
  ...
  const handleDeleteProduct = async (productId: number) => {
    try {
      const response = await fetch(`https://3.120.116.20:9090/api/v1/product/delete/${productId}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${keycloak.token}`,
        },
